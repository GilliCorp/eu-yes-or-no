...
4. **"Haven't we already locked in two-thirds?"** (the myth-buster) — **built ONLY on facts already multi-vote-verified (3-0) in the main ch9 research pass.** Three beats:
   - The ⅔ is *implemented EEA law* that makes accession **faster to complete**, **not** banked/settled terms — Iceland's own MFA: *"no obvious reason to assume… the formal process [could be] accelerated."* *(src: MFA Summary Conclusions — verified)*
   - **Nothing is final until the full Accession Treaty is signed and ratified by every member state + the European Parliament** — so even Iceland's 11 *provisionally*-closed chapters bound nothing. *(src: verified — accession treaty requires unanimous Council + EP consent + ratification by all)*
   - **The hard third was never even negotiated:** fisheries, agriculture, free movement of capital and right of establishment were never opened. *(src: MFA Summary Conclusions; EP briefing — verified)*
   - **EXCLUDED — do NOT publish (single-pass, lower-confidence; deferred to a future verified pass):** the enumerated list of *which* 11 chapters were closed; the "chapters can be reopened" phrasing; and the restart-procedure analysis (suspended-vs-withdrawn, resume-vs-fresh, 2021 methodology remapping). The page must not state these.
...
- **Decision (Gísli, 2026-06-18):** section 4 ships **only** the already-multi-vote-verified core (the three beats above, all from 3-0-verified primary sources in the ch9 pass). The single-pass specifics (enumerated 11-chapter list; "chapters can reopen" phrasing; restart-procedure mechanics) are **deferred** to a future verified research pass and must not appear on the page. Logged as a research backlog item in `overnight-research-plan` memory.
